        ...
        for member in response:  #for loop to iterate through details of each member
            ...
            for repo in repos: #go through all repositories for each user
                ...
                if maximum < repo['stargazers_count']:  #check for repository with max no of stars
                    ...
                    repository = repo['name']           #set repository with max stars

            results.append((user, repository, maximum))   #append the list with required information for each user

    results.sort(key = lambda arr: arr[2], reverse = True)  #sort the list according to no of max stars
