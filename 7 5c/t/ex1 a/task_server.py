"""TODO:
    ...
    * Implement nondestructive_editTask
"""
...
class TaskapiImpl:
    ...
    #TaskapiImpl.__enter__ and TaskapiImpl.__exit__ to save and load tasks to and from a file.
    def __enter__(self):
...
#nondestructive_editTaskk RPC that edits an existing task by keeping the history. 
    def nondestructive_editTask(self, request: wrappers_pb2.StringValue, context) -> task_pb2.Task:    #method to edit tasks
        ...
        with self.lock:   #need a lock to prevent data races. data race occurs when access stored values in "tasks" dictionary
            ...
            previousVersions = self.history[idno]	#get list of previous versions
            previousVersions.append(request.description)	#append the new version
            self.history[idno] = previousVersions	#store the list

            print("Stored versions:")	#print stored versions
            print(self.history[idno])
