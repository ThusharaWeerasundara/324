"""TODO:
    ...
    * Implement destructive_editTask 
"""
...
class TaskapiImpl:
    ...
    #TaskapiImpl.editTask RPC that edits an existing task. 
    def destructive_editTask(self, request: wrappers_pb2.StringValue, context) -> task_pb2.Task:    #method to edit tasks
        ...
        with self.lock:   #need a lock to prevent data races. data race occurs when access stored values in "tasks" dictionary
            ...
            self.tasks.pop(idno)  #delete existing task

            t = task_pb2.Task(id=self.task_id, description=request.description) 	#create a new task with a new id

            print("edited task  :")  #display new task
            print(t)
            self.tasks[self.task_id] = t
            self.task_id += 1 #increment task id
            print("Current tasks: ")
            print(self.tasks)
            return t     #return edited task 
